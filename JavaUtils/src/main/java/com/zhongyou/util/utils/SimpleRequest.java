						...
						() -> {
							synchronized (mRequestLock) {
								...
								try {
									...
									ZyLogger.printException(TAG, new RuntimeException(String.format("Error canceling simple request tagged %s when reaching timeout, calling chain: %s", getTag(), generateCallChainInfo()), e));
								}
								try {
									mCallback.onTimeout();
								} catch (Exception e) {
