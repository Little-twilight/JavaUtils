						...
						() -> {
							synchronized (mRequestLock) {
								...
								try {
									...
									Logger.printException(TAG, new RuntimeException(String.format("Error canceling loop request tagged %s when reaching timeout, calling chain: %s", getTag(), generateCallChainInfo()), e));
								}
								try {
									mCallback.onTimeout();
								} catch (Exception e) {
