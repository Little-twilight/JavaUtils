...
import com.macfred.util.function.Function;
import com.macfred.util.function.TriConsumer;
			...
			handelResult(
					...
					callback -> {
						...
						try {
							...
							Logger.printException(TAG, new RuntimeException(String.format("Error arising rollback for atomic request tagged %s on major action cancel, calling chain: %s", getTag(), generateCallChainInfo()), ee));
						}
					}
			);
		}
			...
			handelResult(
					...
					callback -> {
						...
						try {
							...
							Logger.printException(TAG, new RuntimeException(String.format("Error arising rollback for atomic request tagged %s on major action exception, calling chain: %s", getTag(), generateCallChainInfo()), ee));
						}
					}
			);
		}
			...
			handelResult(
					...
					callback -> {
						...
						try {
							...
							Logger.printException(TAG, new RuntimeException(String.format("Error arising rollback for atomic request tagged %s on major action timeout, calling chain: %s", getTag(), generateCallChainInfo()), e));
						}
					}
			);
		}
		...
		if (mHelper.isResourceAcquired()) {
			try {
				if (!mHelper.releaseResource()) {
					Logger.e(TAG, String.format("Failed to release resource for atomic request tagged %s: %s, calling chain: %s", getTag(), message, generateCallChainInfo()));
				}
			} catch (Exception e) {
				Logger.printException(TAG, new RuntimeException(String.format("Error releasing resource for atomic request tagged %s: %s, calling chain: %s", getTag(), message, generateCallChainInfo()), e));
			}
		}
	}
						...
						() -> {
							synchronized (mRequestLock) {
								...
								try {
									...
									Logger.printException(TAG, new RuntimeException(String.format("Error canceling major action for atomic request tagged %s on reaching timeout, calling chain: %s", getTag(), generateCallChainInfo()), e));
								}
								try {
									ariseRollback();
								} catch (Exception ee) {
									Logger.printException(TAG, new RuntimeException(String.format("Error arising rollback for atomic request tagged %s on reaching timeout, calling chain: %s", getTag(), generateCallChainInfo()), ee));
								}
								try {
	...
	public void cancel() {
		synchronized (mRequestLock) {
			...
			try {
				...
				Logger.printException(TAG, new RuntimeException(String.format("Error arising rollback for atomic request tagged %s on invoking cancel, calling chain: %s", getTag(), generateCallChainInfo()), ee));
			}
		}
	}
					...
					synchronized (mModificationLock) {
						...
						if (previous != null) {
							if (previous == request) {
								...
								Logger.w(TAG, String.format("Trying to release resource %s for atomic request tagged %s when previous request tagged %s unfinished\n Calling chain: %s\n Calling chain for previous request: %s", mResourceInfoGenerator.apply(resource), request.getTag(), previous.getTag(), request.generateCallChainInfo(), previous.generateCallChainInfo()));
								return false;
							}
						} else {
							return false;
